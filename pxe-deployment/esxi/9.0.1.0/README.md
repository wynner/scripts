# ESXi 9.0.1.0 Installer Payload

This folder contains the tracked `boot.cfg` template for ESXi 9.0.1.0.

Do not track Broadcom-owned ESXi binaries, VIBs, or ISO payload files in Git.

At deployment time, customers must copy the installer files from their own licensed ESXi 9.0.1.0 ISO into this folder. The folder must contain `mboot.efi` and every module referenced by `boot.cfg`.
